...
We propose a two-phase deep learning system for predicting in-hospital mortality from ICU data. Phase 1 trains a multimodal contrastive model aligning clinical notes with physiological signal measurements. Phase 2 constructs a heterogeneous temporal graph per ICU stay and trains a Graph Neural Network (GNN) classifier using the pre-trained note embeddings. On 52,727 ICU stays from MIMIC-IV, our system achieves **AUROC 0.816**, **AUPRC 0.379**, and **Brier score 0.182**. A key finding is that the Phase 1 embeddings alone achieve AUROC 0.755 on epoch 1 of GNN training — before any task-specific learning — directly demonstrating the clinical utility of contrastive pre-training.
...
### 5.1 Architecture Ablations (frozen embeddings)

All variants plateau near 0.82 AUROC, confirming the bottleneck is representational (frozen embedding quality), not architectural capacity.
...
### 5.2 End-to-End Fine-tuning (negative result)
...
### 5.3 Phase 1 Contribution Summary

| Model | AUROC | Delta |
|---|---|---|
| Phase 1 linear probe (logistic on note embeddings) | 0.730 | baseline |
| Phase 2 GNN with Phase 1 embeddings (frozen) | **0.816** | **+8.6 pp** |
| Phase 2 GNN with e2e fine-tuning | 0.764 | −5.2 pp vs frozen |
...
2. Constructs heterogeneous temporal graphs from ICU stays and trains a GNN classifier that achieves **AUROC 0.816** and **AUPRC 0.379** on 52,727 patients.
3. Demonstrates through epoch-1 analysis that Phase 1 embeddings account for 0.755 of the final 0.816 AUROC — quantifying the contribution of contrastive pre-training to the downstream task.
4. Identifies the ceiling: frozen embedding quality, not GNN architecture, is the binding constraint at this scale.
